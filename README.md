...
- **Reviewer reports**: Use `/ahead:reviewer-report` (or just ask for a report aimed at a reviewer/external reader) to build a standalone, portable HTML report — embedded CSS, presentational design, sticky decision card, optional interactive per-item decision capture (reviewer picks and copies a summary to paste in the MR) — that translates internal jargon and never links dev-internal docs; distinct from the internal `current-work` dashboard style
